...
int getMaxVal(int arr[], unsigned int n) {
    int max = arr[0];
...
void radixSortPairs(radixSorter* sorter, const uint64_t* keysIn, const uint64_t* keysOut,
    ...
    cudaError_t sorterError = cub::DeviceRadixSort::SortPairs(sorter->tempStorage,sorter->tempStorageBytes,
        keysIn,keysOut,valsIn,valsOut,numCount);

    if(sorterError == 0) {
        printf("ERROR: Error occured with performing radix sort: %s\n", cudaGetErrorString(sorterError));
        exit(-1);
    }
}
